...
<script>
// Replace the entire script section in protocol_edit_drawio.php with this code
    ...
    window.addEventListener('message', function(evt) {
        if (typeof evt.data === 'string' && evt.data.length > 0) {
            try {
                ...
                // Handle export event
                if (msg.event === 'export') {
                    console.log('Export event received from Draw.io');
                    if (msg.format === 'xmlsvg' && msg.xml) {
                        ...
                        // If this was for preview or XML display
                        ...
                        console.log('Export event missing XML content or proper format');
                    }
                }
                ...
                if (msg.event === 'save') {
                    ...
                    if (msg.xml) {
                        console.log('Got XML from save event, length:', msg.xml.length);
                        diagramXml = msg.xml;
                        saveDiagram(diagramXml);
                    } else {
...
document.getElementById('save-diagram-btn').addEventListener('click', function() {
    console.log('Save diagram button clicked');
    ...
    if (iframe && iframe.contentWindow) {
        try {
            // Request the HTML export from the editor
            iframe.contentWindow.postMessage(JSON.stringify({
                action: 'export',
                format: 'html',
                xml: true, // Also include XML for future editing
                spin: 'Saving diagram...',
                embedImages: true // Include images in the HTML
            }), '*');
            ...
            function saveMessageHandler(evt) {
                if (typeof evt.data === 'string' && evt.data.length > 0) {
                    try {
                        ...
                        // Handle export event
                        if (msg.event === 'export') {
                            console.log('Got export response type:', msg.format);
                            ...
                            // Get HTML content
                            if (msg.data) {
                                console.log('Using msg.data for HTML content');
                                htmlContent = msg.data;
                            } else {
                                console.error('No HTML content in response');
                                alert('Failed to get diagram HTML. Please try again.');
                                btn.innerHTML = originalBtnText;
                                btn.disabled = false;
                                return;
                            }
                            
                            // Get XML content for future editing
                            if (msg.xml) {
                                console.log('Using msg.xml for XML content');
                                xmlContent = msg.xml;
                            }
                            ...
                            formData.append('diagram_xml', xmlContent);
                            formData.append('diagram_html', htmlContent);
    ...
    function saveDiagram(diagramXml) {
        ...
        // Generate HTML from the XML (simplified approach)
        let diagramHtml = '<div class="drawio-diagram">';
        diagramHtml += '<div class="diagram-xml" style="display:none;">' + escapeHtml(diagramXml) + '</div>';
        diagramHtml += '<div class="diagram-container">';
        diagramHtml += '<div class="diagram-placeholder">';
        diagramHtml += '<i class="ti ti-chart-arrows" style="font-size: 3rem; color: #3572b0;"></i>';
        diagramHtml += '<p class="mt-2">Interactive Flowchart</p>';
        diagramHtml += '<p class="small text-muted">Loading diagram...</p>';
        diagramHtml += '</div>';
        diagramHtml += '</div>';
        diagramHtml += '</div>';
